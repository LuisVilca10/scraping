...
import { FaAngry, FaImage, FaSmile, FaUser } from "react-icons/fa";
...
const AdminNoticias = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, "noticias"), newNews);
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingNews) {
        ...
        const userDoc = doc(db, "noticias", editingNews.id);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (result.isConfirmed) {
        ...
        const questionDoc = doc(db, "noticias", id);
            ...
            {filteredNoticias.map((noticia) => (
              <tr key={noticia.id}>
                ...
                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                  {noticia.descripcion?.length < 1
                    ? noticia.descripcion // Si tiene menos de 3 caracteres, mostrarla tal cual
                    : noticia.descripcion?.length > 10
                      ? `${noticia.descripcion.substring(0, 10)}...` // Si tiene más de 10 caracteres, recortar a 10
                      : noticia.descripcion // Si tiene entre 3 y 10 caracteres, mostrarla sin cambios
                  }
                </td>
                ...
                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                  ...
                  <FontAwesomeIcon icon={faTrash} onClick={() => handleDelete(noticia.id)} className={"fas fa-tools ml-4 text-sm text-red-600"} />
                </td>
              </tr>
            ))}
          </tbody>
